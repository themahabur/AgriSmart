...
const ExpertAdvice = () => {
  ...
  const fetchExperts = async () => {
    ...
    try {
      ...
      if (data.status) {
        // Filter only experts (for now showing all, later filter by role === 'expert')
        const expertUsers = data.data.users.filter(
          (user) => user.role === "expert" || user.role === "farmer" // Temporary: showing farmers until experts are added
        );
  ...
  useEffect(() => {
    ...
    // Specialty filter (will be used when expert data has specialty field)
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12 text-center md:text-left">
          ...
          <p className="text-lg text-gray-600 font-hind max-w-2xl">
            অভিজ্ঞ কৃষি বিশেষজ্ঞদের কাছ থেকে পেশাদার পরামর্শ নিন এবং আপনার ফসলের
            উন্নতি করুন
          </p>
        ...
        {!loading && !error && filteredExperts.length > 0 && (
          <div className="mt-16 bg-gradient-to-r from-green-600 to-emerald-700 rounded-2xl p-8 md:p-12 text-center shadow-xl">
            ...
            <button className="bg-white text-green-600 font-bold py-3 px-8 rounded-full hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 font-hind">
              পরামর্শ শুরু করুন
            </button>
          </div>
        )}
